...
export class CityInfoService {
  ...
  private cityInfoUrl = "http://10.143.13.41:451/api/info"

  getCityInfoForTimePeriod(data): Observable<CityInfo> {
    return this.http.get(`${this.cityInfoUrl}/${data.cityName}/${data.epoch}/${data.years}`, {
      headers: this.appendCORSHeaders()
    }).map(res => res.json());
  }

  getFullCityInfo(name): Observable<CityFullInfo> {
    return this.http.get(`${this.cityInfoUrl}/${name}`, {
      headers: this.appendCORSHeaders()
    }).map(res => res.json());
  }
