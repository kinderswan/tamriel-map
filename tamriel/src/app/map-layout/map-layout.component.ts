...
export class MapLayoutComponent implements OnInit {
  ...
  private createMapPointTimeElement(mentioned: Array<DateMarker>, timePeriod: TimePeriod) {
    ...
    var requestedDates = mentioned.filter(mention => {
      return mention.Epoch === timePeriod.StartTime.Epoch && mention.Year >= timePeriod.StartTime.Year && mention.Year <= timePeriod.EndTime.Year;
    });
    var span = document.createElement("span");
    span.dataset.epoch = timePeriod.StartTime.Epoch;
    span.dataset.years = requestedDates.map((date) => { return date.Year; }).join(";");
    return span;
  ...
  private onTimePeriodSelected(event: CustomEvent) {
    ...
    this.mapService.getCitiesByTimePeriod(timePeriod.StartTime.Epoch, timePeriod.StartTime.Year, timePeriod.EndTime.Year)
